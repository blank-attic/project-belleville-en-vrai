...
class Task extends BitwiseFlag
{
  ...
  const FLAG_PHOTO = 4;     // BIT #4 of $flags has the value 4
  const FLAG_VIDEO = 8;      // BIT #8 of $flags has the value 8
  const FLAG_CUISINE = 16;      // BIT #16 of $flags has the value 16
  const FLAG_LIVE = 32;      // BIT #32 of $flags has the value 32
  const FLAG_ACCEUIL_PROPRETE = 64;      // BIT #64 of $flags has the value 64
  ...
  public function isLive(){
    return $this->isFlagSet(self::FLAG_LIVE);
  }
  ...
  public function setLive($value){
    $this->setFlag(self::FLAG_LIVE, $value);
  }
